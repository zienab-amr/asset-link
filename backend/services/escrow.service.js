...
const { bookingId, contractId, rentalAmount, securityDeposit, currency } = data;
  ...
  // required fields validation - by Eman
    ...
    throw makeError("rentalAmount is required", 400);
    if (securityDeposit === undefined || securityDeposit === null)
    throw makeError("securityDeposit is required", 400);
  ...
  // amounts must be non-negative - by Eman
    if (Number(rentalAmount) < 0)
        throw makeError("rentalAmount cannot be negative", 400);
    if (Number(securityDeposit) < 0)
        throw makeError("securityDeposit cannot be negative", 400);
