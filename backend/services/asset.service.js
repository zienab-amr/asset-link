...
const generateCode = async () => {
  const lastAsset = await assetModel
    .findOne()
    .sort({ createdAt: -1 });
...
const addAsset = async (assetData) => {
  const {
    companyId, assetCategoryId,
    assetName, assetImages, description, price,
    status, location
  } = assetData;
  ...
  if (!assetImages || assetImages.length === 0) throw new Error('Image of asset is required');
  if (!description) throw new Error('Description is required');
  if (!price || !price.daily) throw new Error('please add at least daily price');

  const assetCode = await generateCode();
  
  const newAsset = new assetModel({
    assetCode, companyId, assetCategoryId,
    assetName, assetImages, description, price,
    status, location
  });
